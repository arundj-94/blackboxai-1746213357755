...
try {
    ...
    foreach ($repeats as $repeat) {
        ...
        if ($nextDate && $nextDate <= $todayStr) {
            ...
            $insertStmt = $pdo->prepare("INSERT INTO tasks (title, description, start_date, due_date, priority, status, category_id, order_index) VALUES (?, ?, ?, ?, ?, 'Pending', ?, (SELECT COALESCE(MAX(order_index), 0) + 1 FROM tasks))");
            $insertStmt->execute([
                $repeat['title'],
                $repeat['description'],
                $nextDate,
